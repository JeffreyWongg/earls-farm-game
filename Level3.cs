...
namespace Final_Project___Jeffrey_Wong_ICS3U
{
    ...
    {
        Rectangle playerRect, enemy1Rect, enemy2Rect, enemy3Rect, enemy4Rect, barrierRect;
        Rectangle[] wall;
        List<Rectangle> crops;
        Image grass, fences, farmer, child1, child2, child3, child4, carrot, eggplant, haybale;
        Image[] playerMovements, enemyMovements;
        Timer playerTimer, enemy1Timer, enemy2Timer, enemy3Timer, enemy4Timer, clockTimer;
        ...
        string[] row;
        int dx, dy;
        int playerSteps, enemy1Steps, enemy2Steps, enemy3Steps, enemy4Steps;
        ...
        private void Level3_Load(object sender, EventArgs e)
        {
            ...
            child3 = Image.FromFile(Application.StartupPath + @"\child_walk_down1.png", true); // The default stance of the child
            child4 = Image.FromFile(Application.StartupPath + @"\child_walk_down1.png", true); // The default stance of the child
            haybale = Image.FromFile(Application.StartupPath + @"\haybale.png", true); // The image of the haybale (barrier)
            carrot = Image.FromFile(Application.StartupPath + @"\carrot.png", true); // The image of a crop (carrot)
            eggplant = Image.FromFile(Application.StartupPath + @"\eggplant.png", true); // The image of a crop (eggplant)
            ...
            // Setting up the timer for the enemy3's movements
            enemy3Timer = new Timer();
            enemy3Timer.Interval = 1000 / 10;
            enemy3Timer.Tick += Enemy3Timer_Tick;

            // Setting up the timer for the enemy4's movements
            enemy4Timer = new Timer();
            enemy4Timer.Interval = 1000 / 10;
            enemy4Timer.Tick += Enemy4Timer_Tick;
            ...
            storyLabel.Text = "The summer break is coming to a close and Earl is anticipating the children's final invasion. They have hit their growth spurt and gained another member.\n\nLets save Earl's farm!";
        ...
        private void StoryButton_Click(object sender, EventArgs e)
        {
            ...
            enemy1Rect = new Rectangle(55, 50, 40, 50);
            enemy2Rect = new Rectangle(655, 50, 40, 50);
            enemy3Rect = new Rectangle(655, 650, 40, 50);
            enemy4Rect = new Rectangle(55, 650, 40, 50);
            ...
            menuLabel.Text = ("\nBatch 1: " + batch1Counter + "/20\n" + "Batch 2: " + batch2Counter + "/52");
            ...
            DrawWalls(); // drawing the walls for batch 1
        ...
        private void Level3_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Space) // if the spacebar is pressed
            {
        ...
        {
            if (batch1Counter >= 20 && inBatch1) // if the player has collected all 24 crops in batch 1
            {
            ...
            if (batch2Counter >= 52) // if the player has collected all the crops in batch 2
            {
                crops.Clear(); // clear the crops list
                // stop all timers
                enemy1Timer.Stop();
                ...
                nextLevelLabel.Text = ("Level 3 Complete!");
            ...
            if (playerRect.IntersectsWith(enemy1Rect) || playerRect.IntersectsWith(enemy2Rect) || playerRect.IntersectsWith(enemy3Rect) || playerRect.IntersectsWith(enemy4Rect)) // if the player intersects with either enemy, load the game over screen
            {
            ...
            for (int i = 0; i < crops.Count; i++) // iterating through the crops list
            {
                if (inBatch1) // if the player is in batch 1
                {
                    if (playerRect.IntersectsWith(crops[i])) // if the player intersects with a crop while in batch 1
                    {
                        ...
                        menuLabel.Text = ("\nBatch 1: " + batch1Counter + "/20\n" + "Batch 2: " + batch2Counter + "/52"); // update the scoreboard/menu with the new batch 1 counter value
                    }
                }
                if (inBatch2) // if the player is in batch 2
                {
                    if (playerRect.IntersectsWith(crops[i])) // if the player intersects with a crop while in batch 2
                    {
                        ...
                        menuLabel.Text = ("\nBatch 1: " + batch1Counter + "/20\n" + "Batch 2: " + batch2Counter + "/52"); // update the scoreboard/menu with the new batch 2 counter value
                    }
                }
            }
            ...
            else if (faceDown && placeBarrier) // if the player is facing down and places a barrier
            {
        ...
        // If the player clicks the "next level" button, load victory screen!
        private void NextLevelButton_Click(object sender, EventArgs e)
        {
            VictoryScreenLoad(new VictoryScreen());
        }

        // This is used to move enemy 1 continuously in a set path
        private void Enemy1Timer_Tick(object sender, EventArgs e)
        {
            if (enemy1Rect.X <= 55 && enemy1Rect.Y <= 450) // these are the coordinates in which the enemy will be moving downward
            {
                AnimateEnemy1Movement(0, 3); // animates moving down
                enemy1Rect.Y += 20; // moves the enemy 10 pixels down
            }
            if (enemy1Rect.X >= 55 && enemy1Rect.Y >= 450) // these are the coordinates in which the enemy will be moving rightward
            {
                AnimateEnemy1Movement(8, 11); // animates moving right
                enemy1Rect.X += 20; // moves the enemy 10 pixels right
            }
            if (enemy1Rect.X >= 455 && enemy1Rect.Y >= 50) // these are the coordinates in which the enemy will be moving upward 
            {
                AnimateEnemy1Movement(12, 15); // animates moving up
                enemy1Rect.Y -= 20; // moves the enemy 10 pixels up
            }
            if (enemy1Rect.X <= 455 && enemy1Rect.Y <= 50) // these are the coordinates in which the enemy will be moving leftward
            {
                AnimateEnemy1Movement(4, 7); // animates moving left
                enemy1Rect.X -= 20; // moves the enemy 10 pixels left
            }
        ...
        {
            if (enemy2Rect.X >= 655 && enemy2Rect.Y <= 450) // these are the coordinates in which the enemy will be moving downward
            {
                AnimateEnemy2Movement(0, 3); // animates moving down
                enemy2Rect.Y += 20; // moves the enemy 10 pixels down
            }
            if (enemy2Rect.X <= 655 && enemy2Rect.Y >= 450) // these are the coordinates in which the enemy will be moving upward 
            {
                AnimateEnemy2Movement(4, 7); // animates moving left
                enemy2Rect.X -= 20; // moves the enemy 10 pixels left
            }
            if (enemy2Rect.X <= 255 && enemy2Rect.Y <= 450) // these are the coordinates in which the enemy will be moving upward 
            {
                AnimateEnemy2Movement(12, 15); // animates moving up
                enemy2Rect.Y -= 20; // moves the enemy 10 pixels up
            }
            if (enemy2Rect.X >= 255 && enemy2Rect.Y <= 50) // these are the coordinates in which the enemy will be moving rightward
            {
                AnimateEnemy2Movement(8, 11); // animates moving right
                enemy2Rect.X += 20; // moves the enemy 10 pixels right
            }
        ...
        // This is used to move enemy 3 continuously in a set path
        private void Enemy3Timer_Tick(object sender, EventArgs e)
        {
            if (enemy3Rect.X >= 655 && enemy3Rect.Y >= 250) // these are the coordinates in which the enemy will be moving upward 
            {
                AnimateEnemy3Movement(12, 15); // animates moving up
                enemy3Rect.Y -= 20; // moves the enemy 10 pixels up
            }
            if (enemy3Rect.X <= 655 && enemy3Rect.Y <= 250) // these are the coordinates in which the enemy will be moving upward 
            {
                AnimateEnemy3Movement(4, 7); // animates moving left
                enemy3Rect.X -= 20; // moves the enemy 10 pixels left
            }
            if (enemy3Rect.X <= 255 && enemy3Rect.Y >= 250) // these are the coordinates in which the enemy will be moving downward
            {
                AnimateEnemy3Movement(0, 3); // animates moving down
                enemy3Rect.Y += 20; // moves the enemy 10 pixels down
            }
            if (enemy3Rect.X >= 255 && enemy3Rect.Y >= 650) // these are the coordinates in which the enemy will be moving rightward
            {
                AnimateEnemy3Movement(8, 11); // animates moving right
                enemy3Rect.X += 20; // moves the enemy 10 pixels right
            }
            if (enemy3Rect.IntersectsWith(playerRect)) // if the enemy intersects with the player, load the game over screen
            {
                GameOverLoad(new GameOver());
            }
            if (enemy3Rect.IntersectsWith(barrierRect)) // stop the enemy's timer and movement if it intersects with a barrier placed by the player
            {
                enemy3Timer.Stop();
            }
            this.Invalidate(); // refreshes the screen
        ...
        // This is used to move enemy 4 continuously in a set path
        private void Enemy4Timer_Tick(object sender, EventArgs e)
        {
            if (enemy4Rect.X <= 55 && enemy4Rect.Y >= 250) // these are the coordinates in which the enemy will be moving upward 
            {
                AnimateEnemy4Movement(12, 15); // animates moving up
                enemy4Rect.Y -= 20; // moves the enemy 10 pixels up
            }
            if (enemy4Rect.X >= 55 && enemy4Rect.Y <= 250) // these are the coordinates in which the enemy will be moving rightward
            {
                AnimateEnemy4Movement(8, 11); // animates moving right
                enemy4Rect.X += 20; // moves the enemy 10 pixels right
            }
            if (enemy4Rect.X >= 455 && enemy4Rect.Y >= 250) // these are the coordinates in which the enemy will be moving downward
            {
                AnimateEnemy4Movement(0, 3); // animates moving down
                enemy4Rect.Y += 20; // moves the enemy 10 pixels down
            }
            if (enemy4Rect.X <= 455 && enemy4Rect.Y >= 650) // these are the coordinates in which the enemy will be moving upward 
            {
                AnimateEnemy4Movement(4, 7); // animates moving left
                enemy4Rect.X -= 20; // moves the enemy 10 pixels left
            }       
            if (enemy4Rect.IntersectsWith(playerRect)) // if the enemy intersects with the player, load the game over screen
            {
                GameOverLoad(new GameOver());
            }
            if (enemy4Rect.IntersectsWith(barrierRect)) // stop the enemy's timer and movement if it intersects with a barrier placed by the player
            {
                enemy4Timer.Stop();
            }
            this.Invalidate(); // refreshes the screen
        ...
        // Plays through an array of images to animate the enemy's movements
        private void AnimateEnemy3Movement(int start, int end)
        {
            enemy3Steps++; // add one to enemy steps to iterate through the desired numbers

            if (enemy3Steps > end || enemy3Steps < start) // if the steps value is not within the range of the parameters
            {
                enemy3Steps = start; // force steps to go to the start
            }

            child3 = enemyMovements[enemy3Steps]; // changing the image of child3 based on the index of the steps
        }

        // Plays through an array of images to animate the enemy's movements
        private void AnimateEnemy4Movement(int start, int end)
        {
            enemy4Steps++; // add one to enemy steps to iterate through the desired numbers

            if (enemy4Steps > end || enemy4Steps < start) // if the steps value is not within the range of the parameters
            {
                enemy4Steps = start; // force steps to go to the start
            }

            child4 = enemyMovements[enemy4Steps]; // changing the image of child3 based on the index of the steps
        }
        ...
        private void Level3_Paint(object sender, PaintEventArgs e)
        {
            ...
            e.Graphics.DrawImage(child3, enemy3Rect); // painting the third child (child3)
            e.Graphics.DrawImage(child4, enemy4Rect); // painting the fourth child (child4)
            e.Graphics.DrawImage(haybale, barrierRect); // painting the barrier

            // this for loop will iterate through the wall array to print all of the walls necessary
            ...
            for (int i = 0; i < crops.Count; i++)
            {
                ...
                {
                    e.Graphics.DrawImage(carrot, crops[i]); // print every crop as a tomato
                }
                if (inBatch2) // if the player is collecting from batch 2
                {
                    e.Graphics.DrawImage(eggplant, crops[i]); // print every crop as cabbage
                }
            }
        }
        ...
        private void DrawWalls()
        {
            ...
            if (inBatch1) // if the player is still collecting the first batch of crops
            {
                ...
                // the following is how level 3, batch 1 will look
                row[0] =  "111111111111111";
                row[1] =  "122000000000221";
                row[2] =  "121110101011121";
                row[3] =  "101210121012101";
                row[4] =  "101010111010101";
                row[5] =  "100000000000001";
                row[6] =  "101110101011101";
                row[7] =  "100210000012001";
                row[8] =  "101110101011101";
                row[9] =  "100000000000001";
                row[10] = "101010111010101";
                row[11] = "101210121012101";
                row[12] = "121110101011121";
            ...
            {
                // 0s represent walkable space, 1s represent a wall, 2s represent a collectable crop, each taking up their own block of 50 x 50
                // the following is how level 3, batch 2 will look
                row[0] =  "111111111111111";
                row[1] =  "122222222222221";
                row[2] =  "101110101011101";
                row[3] =  "101010101010101";
                row[4] =  "101010111010101";
                row[5] =  "122222222222221";
                row[6] =  "101110101011101";
                row[7] =  "100010000010001";
                row[8] =  "101110101011101";
                row[9] =  "122222222222221";
                row[10] = "101010111010101";
                row[11] = "101010101010101";
                row[12] = "101110101011101";
                row[13] = "122222222222221";
                row[14] = "111111111111111";
                ...
                for (int i = 0; i < row.Length; i++) // this for loop will represent how many rows there are
                {
                    for (int j = 0; j < row.Length; j++) // iterating through every column
                    {
                        if (row[i].Substring(j, 1) == "1") // if any of the row's characters is a "1", we will print a fence
                        {
                            wall[wallCounter] = new Rectangle((j * 50) + 5, (i * 50), 40, 50); // adding a new rectangle to the wall array based on where the 1 is placed
                        }
                    }
                }
                for (int i = 0; i < row.Length; i++) // this for loop will represent how many rows there are
        ...
        // Loads the next level (level 3)
        private void VictoryScreenLoad(Form victoryScreen)
        {
            backgroundMusic.stop(); // stops the music
            victoryScreen.FormClosed += VictoryScreen_FormClosed;// if the form is closed, the application will also close
            this.Hide(); // hide the current form
            victoryScreen.Show(); // show the form from the parameters (level 3)
        }
        ...
        // If the victory screen form is closed, exit the application
