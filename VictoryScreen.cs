...
namespace Final_Project___Jeffrey_Wong_ICS3U
{
    public partial class VictoryScreen : Form
    {
        ...
        private void VictoryScreen_Load(object sender, EventArgs e)
        {
            ...
            this.MaximumSize = new Size(1440, 900);
            this.Width = 1440;
            this.Height = 900;
            this.Text = "Final Project";
            this.CenterToScreen();

            farm = Image.FromFile(Application.StartupPath + @"\farm_day.jpg", true); // The farm image is for the background in the starting screen
            ...
            // Setting up the sound player for the celebration music
            happyMusic = new AudioFilePlayer();
            if (happyMusic.setAudioFile(Application.StartupPath + @"\happy_music.mp3"))
            {
                happyMusic.playLooping();
            }
            ...
            titleLabel.Text = "You have saved Earl's Farm!";
            ...
            replayButton.Click += ReplayButton_Click; // A new event for when the replay button is clicked
            exitButton.Click += ExitButton_Click; // a new event for when the exit button is clicked
