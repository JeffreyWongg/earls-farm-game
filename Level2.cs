...
namespace Final_Project___Jeffrey_Wong_ICS3U
{
    ...
    {
        Rectangle playerRect, enemy1Rect, enemy2Rect, enemy3Rect, barrierRect;
        Rectangle[] wall;
        List<Rectangle> crops;
        Image grass, fences, farmer, child1, child2, child3, cabbage, tomato, haybale;
        Image[] playerMovements, enemyMovements;
        Timer playerTimer, enemy1Timer, enemy2Timer, enemy3Timer, clockTimer;
        ...
        string[] row;
        int dx, dy;
        int playerSteps, enemy1Steps, enemy2Steps, enemy3Steps;
        ...
        private void Level2_Load(object sender, EventArgs e)
        {
            ...
            tomato = Image.FromFile(Application.StartupPath + @"\tomato.png", true); // The image of a crop (tomato)
            cabbage = Image.FromFile(Application.StartupPath + @"\cabbage.png", true); // The image of a crop (cabbage)
            ...
            this.KeyUp += Level2_KeyUp;
            this.KeyDown += Level2_KeyDown;
            ...
            storyLabel.Text = "It has been a few weeks since the last invasion. It is now the middle of summer, and the kids are back at it. They appear to be faster and there are more of them.\n\nAre you ready to save Earl's farm?";
        ...
        private void StoryButton_Click(object sender, EventArgs e)
        {
            ...
            enemy1Rect = new Rectangle(655, 100, 40, 50);
            enemy2Rect = new Rectangle(505, 200, 40, 50);
            enemy3Rect = new Rectangle(405, 50, 40, 50);
        ...
        private void PlayerTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (batch2Counter >= 21) // if the player has collected all the crops in batch 2
            {
                ...
                nextLevelLabel.Text = ("Level 2 Complete!");
            ...
            if (playerRect.IntersectsWith(enemy1Rect) || playerRect.IntersectsWith(enemy2Rect) || playerRect.IntersectsWith(enemy3Rect)) // if the player intersects with either enemy, load the game over screen
            {
        ...
        // If the player clicks the "next level" button, load level 3
        private void NextLevelButton_Click(object sender, EventArgs e)
        {
            Level3Load(new Level3());
        }

        // This is used to move enemy 1 continuously in a set path
        private void Enemy1Timer_Tick(object sender, EventArgs e)
        {
            if (enemy1Rect.X >= 655 && enemy1Rect.Y <= 600) // these are the coordinates in which the enemy will be moving downward
            {
                AnimateEnemy1Movement(0, 3); // animates moving down
                enemy1Rect.Y += 20; // moves the enemy 10 pixels down
            }
            if (enemy1Rect.X <= 655 && enemy1Rect.Y >= 600) // these are the coordinates in which the enemy will be moving leftward
            {
                AnimateEnemy1Movement(4, 7); // animates moving left
                enemy1Rect.X -= 20; // moves the enemy 10 pixels left
            }
            if (enemy1Rect.X <= 55 && enemy1Rect.Y <= 600) // these are the coordinates in which the enemy will be moving upward 
            {
                AnimateEnemy1Movement(12, 15); // animates moving up
                enemy1Rect.Y -= 20; // moves the enemy 10 pixels up
            }
            if (enemy1Rect.X >= 55 && enemy1Rect.Y <= 100) // these are the coordinates in which the enemy will be moving rightward
            {
        ...
        {
            if (enemy3Rect.X >= 405 && enemy3Rect.Y <= 650) // these are the coordinates in which the enemy will be moving downward
            {
                AnimateEnemy3Movement(0, 3); // animates moving down
                enemy3Rect.Y += 20; // moves the enemy 10 pixels down
            }
            if (enemy3Rect.X <= 405 && enemy3Rect.Y >= 650) // these are the coordinates in which the enemy will be moving upward 
            {
                AnimateEnemy3Movement(4, 7); // animates moving left
                enemy3Rect.X -= 20; // moves the enemy 10 pixels left
            }
            if (enemy3Rect.X <= 305 && enemy3Rect.Y <= 650) // these are the coordinates in which the enemy will be moving upward 
            {
                AnimateEnemy3Movement(12, 15); // animates moving up
                enemy3Rect.Y -= 20; // moves the enemy 10 pixels up
            }
            if (enemy3Rect.X >= 305 && enemy3Rect.Y <= 50) // these are the coordinates in which the enemy will be moving rightward
            {
        ...
        // The paint functoin to display all the graphics and images
        private void Level2_Paint(object sender, PaintEventArgs e)
        {
            ...
            for (int i = 0; i < crops.Count; i++)
            {
                ...
                {
                    e.Graphics.DrawImage(tomato, crops[i]); // print every crop as a tomato
                }
                if (inBatch2) // if the player is collecting from batch 2
                {
                    e.Graphics.DrawImage(cabbage, crops[i]); // print every crop as cabbage
                }
            }
        }
        ...
        private void DrawWalls()
        {
            ...
            if (inBatch1) // if the player is still collecting the first batch of crops
            {
                ...
                // the following is how level 2, batch 1 will look
                row[0] =  "111111111111111";
                row[1] =  "121201000102121";
                row[2] =  "100000020000001";
                row[3] =  "101021000120101";
                row[4] =  "100000020000001";
                row[5] =  "101201000102101";
                row[6] =  "100000020000001";
                row[7] =  "101021000120101";
                row[8] =  "100000020000001";
                row[9] =  "101201000102101";
                row[10] = "100000020000001";
                row[11] = "101021000120101";
                row[12] = "100000020000001";
                row[13] = "121201000102121";
            ...
            {
                // 0s represent walkable space, 1s represent a wall, 2s represent a collectable crop, each taking up their own block of 50 x 50
                // the following is how level 2, batch 2 will look
                row[0] =  "111111111111111";
                row[1] =  "101001222100101";
                row[2] =  "100002000200001";
                row[3] =  "101001000100101";
                row[4] =  "100200000002001";
                row[5] =  "101001000100101";
                row[6] =  "120000020000021";
                row[7] =  "101001020100101";
                row[8] =  "120000020000021";
                row[9] =  "101001000100101";
                row[10] = "100200000002001";
                row[11] = "101001000100101";
                row[12] = "100002000200001";
                row[13] = "101001222100101";
                row[14] = "111111111111111";
        ...
        private void Level3Load(Form level3)
        {
            backgroundMusic.stop(); // stops the music
            level3.FormClosed += Level3_FormClosed; // if the form is closed, the application will also close
            this.Hide(); // hide the current form
            level3.Show(); // show the form from the parameters (level 3)
        }
        ...
        // If the level 3 form is closed, exit the application
